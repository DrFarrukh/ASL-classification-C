...
class SensorDataProcessor:
    def __init__(self, model_path, window_size=91, confidence_threshold=0.3, use_jit=True, output_file=None, slow_mode=False):
        ...
        self.sample_count = 0
        self.target_samples = 91
        self.sample_rate = 5.0  # seconds to collect 91 samples
        
        print(f"Initializing SensorDataProcessor with:")
    ...
    def _read_process_output(self):
        ...
        while self.running:
            try:
                ...
                # In slow mode, control the rate of data collection
                if self.slow_mode:
                    self.sample_count += 1
                    if self.sample_count >= self.target_samples:
                        # Sleep to achieve the desired rate
                        time.sleep(self.sample_rate / self.target_samples)
                        self.sample_count = 0
    ...
    def _check_and_process_data(self):
        ...
        if all(size >= self.window_size for size in buffer_sizes):
            ...
            # Update output file with buffer status
            if self.output_file:
                with open(self.output_file, 'r') as f:
                    data = json.load(f)
                
                data["buffers"] = buffer_sizes
        ...
        while self.running:
            try:
                ...
                if self.output_file:
                    ...
                    data["prediction"] = self.class_names[pred_idx]
...
def main():
    ...
    parser.add_argument('--output-file', type=str, default="/tmp/asl_output.json",
                        help="JSON file to write output to (for watch command)")
    parser.add_argument('--slow', action='store_true',
                        help="Slow down data collection to collect 91 samples in 5 seconds")
